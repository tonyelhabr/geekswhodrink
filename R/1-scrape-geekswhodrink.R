...
scrape_and_bind_geekswhodrink_venue_quiz_results <- function(
  ...
  if (!is.null(res_existing$error)) {
    ...
    existing_quiz_results <- data.frame()
  ...
  if (n_new_records == 0) {
    if (isFALSE(release_file_exists)) {
      cli::cli_inform(c('i' = 'Writing to release file even since no release file existed before.'))
      write_geekswhodrink_release(
        res,
        name = venue_id, 
        tag = 'venue-quiz-results'
...
cli::cli_inform('Scraping quiz results from locations with prior quiz results.')
n_venues <- length(venues$venue_id)
new_quiz_results <- purrr::map_dfr(
    11:n_venues,
    \(.x) {
      cli::cli_inform('Scraping {.x} of {n_venues} previously scraped venues.')
